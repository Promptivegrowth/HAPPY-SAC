...
async function run() {
    try {
        ...
        // 2. Kardex
        console.log("-> Registrando Entradas en Kardex...");
        ...
        const recipe = (await sup('/recipes', 'POST', {
            ...
            estado: 'ACTIVA',
            descripcion: 'Manufactura de Traje Spiderman Advanced con materiales elásticos y triple costura.',
            costos_indirectos: 40.0,
            merma_default: 4.0
        }))[0];

        // 6. Items Receta
        console.log("-> Añadiendo Insumos a la Ficha...");
        await sup('/recipe_items', 'POST', [
            { recipe_id: recipe.id, material_id: m1.id, cantidad: 1.70 },
            { recipe_id: recipe.id, material_id: m2.id, cantidad: 1.10 }
        ]);

        // 7. Operaciones
        console.log("-> Añadiendo Operaciones de Producción...");
        await sup('/recipe_operations', 'POST', [
            { recipe_id: recipe.id, tipo_operacion: 'CORTE LASER', costo_base: 25.0, orden: 1 },
            { recipe_id: recipe.id, tipo_operacion: 'COSTURA REFORZADA', costo_base: 70.0, orden: 2 },
            { recipe_id: recipe.id, tipo_operacion: 'OJAL Y BOTON', costo_base: 15.0, orden: 3 },
            { recipe_id: recipe.id, tipo_operacion: 'LAMINADO', costo_base: 10.0, orden: 4 }
        ]);

        console.log("¡TERMINADO! Data industrial poblada 100% exitosamente.");
    } catch (e) {
