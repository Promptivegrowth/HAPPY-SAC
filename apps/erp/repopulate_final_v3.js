...
async function run() {
    try {
        console.log("Starting FINAL Data Population v3...");

        // 1. Create Materials
        console.log("Materials...");
        const materials = await sup('/products', 'POST', [
            { nombre: 'Gasa Spiderman Roja V3', codigo: 'MAT-SPD-V3-01', tipo_item: 'MATERIAL', precio: 32.0, existencia: 500, categoria_id: CAT_MAT, company_id: COMPANY_ID },
            { nombre: 'Gasa Spiderman Azul V3', codigo: 'MAT-SPD-V3-02', tipo_item: 'MATERIAL', precio: 32.0, existencia: 400, categoria_id: CAT_MAT, company_id: COMPANY_ID }
        ]);
        ...
        const recipe = (await sup('/recipes', 'POST', {
            ...
            estado: 'ACTIVA',
            descripcion: 'Ficha Técnica Principal Spiderman 2026 V3',
        ...
        await sup('/recipe_items', 'POST', [
            { recipe_id: recipe.id, material_id: materials[0].id, cantidad: 1.5 },
            { recipe_id: recipe.id, material_id: materials[1].id, cantidad: 1.0 }
        ]);
        ...
        try {
            await sup('/recipe_operations', 'POST', [
                ...
                { recipe_id: recipe.id, tipo_operacion: 'CONFECCION', costo_base: 55.0 }
            ]);
        } catch (e) {
